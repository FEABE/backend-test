...
- 우선 제가 해본 프로젝트 대부분은 엔터프라이즈급이 아니기 때문에 JWT로 구현을 주로 하였습니다. 내부에서만 사용하는거다 보니 ec2에 도커로 서버를 올린 후 redis 이미지를 활용하여 RDB가 아닌 redis에서 토큰 관리를 진행하였습니다. 구현을 진행하면서 넓은 의미의 백오피스 구축이 아닌 사용자와 상호 작용하고 내부 관리에도 영향을 줄 수 있는 이너오피스 라는 범위로 구축을 하였습니다.
